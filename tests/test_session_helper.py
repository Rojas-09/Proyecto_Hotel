...
class TestLoginRequired:
    ...
    def test_sin_sesion(self, app):
        with patch("app.utils.session_helper.session", {}), patch(
            "app.utils.session_helper.flash"
        ) as mf, patch("app.utils.session_helper.redirect") as mr, patch(
            "app.utils.session_helper.url_for"
        ) as mu:
    ...
    def test_con_sesion_valida(self, app):
        with patch("app.utils.session_helper.session", {"user_id": 1}), patch(
            "app.utils.session_helper._revalidar_usuario", return_value=True
        ):
            assert self.decorated() == "ok"

    def test_usuario_desactivado(self, app):
        with patch("app.utils.session_helper.session", {"user_id": 1}), patch(
            "app.utils.session_helper._revalidar_usuario", return_value=False
        ), patch("app.utils.session_helper.flash") as mf, patch(
            "app.utils.session_helper.redirect"
        ) as mr, patch(
            "app.utils.session_helper.url_for"
        ) as mu:
            mr.return_value = "redirected"
...
class TestRolesAllowed:
    ...
    def test_usuario_desactivado(self, app):
        with patch(
            "app.utils.session_helper.session", {"user_id": 1, "user_rol": "admin"}
        ), patch(
            "app.utils.session_helper._revalidar_usuario", return_value=False
        ), patch(
            "app.utils.session_helper.flash"
    ...
    def test_rol_no_permitido(self, app):
        with patch(
            "app.utils.session_helper.session", {"user_id": 1, "user_rol": "cliente"}
        ), patch(
            "app.utils.session_helper._revalidar_usuario", return_value=True
        ), patch(
            "app.utils.session_helper.flash"
    ...
    def test_rol_permitido(self, app):
        with patch(
            "app.utils.session_helper.session", {"user_id": 1, "user_rol": "admin"}
        ), patch("app.utils.session_helper._revalidar_usuario", return_value=True):
            assert self.decorated() == "ok"
