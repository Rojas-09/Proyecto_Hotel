...
def seed(app):
    ...
    with app.app_context():
        admin = Usuario(
            nombre="Admin", apellido="Root", email="admin@h.com", rol=RolEnum.admin
        )
        admin.password = "Admin123!"
        db.session.add(admin)
        cli = Usuario(
            nombre="Juan", apellido="Pérez", email="juan@h.com", rol=RolEnum.cliente
        )
        ...
        db.session.add(hab)
        db.session.commit()
        return {"habitacion_id": hab.id, "huesped_id": huesped.id, "usuario_id": cli.id}
...
class TestFlujoCompleto:
    ...
    def test_flujo_integral(self, client, app, seed):
        with app.app_context():
            ...
            # 3. Check-in
            r = client.put(f"/api/v1/reservas/{rid}/checkin", headers=admin_h)
            ...
            # 7. Check-out
            r = client.put(f"/api/v1/reservas/{rid}/checkout", headers=admin_h)
            ...
            # 9. Emitir factura
            r = client.post(f"/api/v1/facturas/reserva/{rid}/emitir", headers=admin_h)
            assert r.status_code == 201, f"Emitir factura: {r.get_json()}"
            assert r.get_json()["data"]["estado"] == "Emitida"

            # 10. Descargar PDF
            r = client.get(f"/api/v1/facturas/reserva/{rid}/descargar", headers=admin_h)
