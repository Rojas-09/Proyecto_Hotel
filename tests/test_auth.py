...
def usuario_cliente(db):
    u = Usuario(nombre="Ana", apellido="García", email="ana@test.com", rol="cliente")
...
def usuario_admin(db):
    u = Usuario(nombre="Admin", apellido="Hotel", email="admin@test.com", rol="admin")
...
class TestRegister:
    ...
    def test_registro_exitoso(self, client):
        res = client.post("/api/v1/auth/register", json={
            ...
            "email": "juan@test.com", "password": "Segura123"
        })
        data = res.get_json()
    ...
    def test_registro_email_duplicado(self, client, usuario_cliente):
        res = client.post("/api/v1/auth/register", json={
            "nombre": "Otra", "apellido": "Persona",
            "email": "ana@test.com", "password": "OtraPass123"
        })
    ...
    def test_registro_campo_faltante(self, client):
        res = client.post("/api/v1/auth/register", json={
            "nombre": "Test", "email": "otro@test.com"
        })
        assert res.status_code == 400
...
class TestLogin:
    ...
    def test_login_password_incorrecta(self, client, usuario_cliente):
        res = client.post("/api/v1/auth/login", json={
            "email": "ana@test.com", "password": "Incorrecta99"
        })
    ...
    def test_login_email_inexistente(self, client):
        res = client.post("/api/v1/auth/login", json={
            "email": "noexiste@test.com", "password": "Cualquiera1"
        })
        assert res.status_code == 401
    ...
    def test_admin_puede_crear_recepcionista(self, client, usuario_admin):
        ...
        res = client.post("/api/v1/auth/usuarios",
                          json={
                              ...
                              "password": "Recep1234", "rol": "recepcionista"
                          },
                          headers={"Authorization": f"Bearer {token}"})
        data = res.get_json()
        assert res.status_code == 201
