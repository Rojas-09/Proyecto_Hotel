...
with app.app_context():
    ...
    r = post('/api/v1/auth/register-admin', json={
        'nombre':'SmokeAdmin','apellido':'X','email':'smoke_admin@example.com','password':'Pass1234!'
    })
    ...
    rh = post('/api/v1/habitaciones/', json={
        'numero':'9101','tipo':'doble','precio_noche':100000,'capacidad':2
    }, token=token)
    print('status', rh.status_code, rh.get_json())

    # Register cliente
    print('\n--- Register cliente ---')
    rc = post('/api/v1/auth/register', json={
        'nombre':'Smoke','apellido':'Cli','email':'smoke_cli@example.com','password':'Pass1234!','documento_id':'CC99999'
    })
    print('status', rc.status_code, rc.get_json())

    # Login cliente
    print('\n--- Login cliente ---')
    rl = post('/api/v1/auth/login', json={'email':'smoke_cli@example.com','password':'Pass1234!'})
    ...
    rres = post('/api/v1/reservas/', json={
        'id_habitacion':1,'fecha_entrada':hoy.isoformat(),'fecha_salida':(hoy+timedelta(days=2)).isoformat(),
        'id_huesped':1
    }, token=token_cli)
    ...
    rgar = post(f'/api/v1/pagos/garantia/{reserva_id}', json={'metodo':'Efectivo'}, token=token_cli)
    ...
    print('\n--- Agregar servicio ---')
    ras = post(f'/api/v1/reservas/{reserva_id}/servicios', json={'tipo':'Comedor','descripcion':'Desayuno smoke','costo':15000}, token=token)
    ...
    rpago = post(f'/api/v1/pagos/liquidacion/{reserva_id}', json={'metodo':'Efectivo'}, token=token)
    ...
    print('\n--- Generar reporte ingresos ---')
    rr = get('/api/v1/reportes/ingresos?fecha_inicio=2020-01-01&fecha_fin=2030-01-01&formato=xlsx', token=token)
